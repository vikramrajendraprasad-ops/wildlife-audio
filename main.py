...
class WildlifeStudio(MDApp):
    ...
    def install_engine(self):
        ...
        if os.path.exists(source):
            ...
            Clock.schedule_once(lambda x: self.engine_ready(False, "libffmpeg_engine.so not found! Did you upload it?"))

    def engine_ready(self, success, error=""):
        if success:
            self.status_btn.text = "Engine Active!"
            self.load_files()
        else:
